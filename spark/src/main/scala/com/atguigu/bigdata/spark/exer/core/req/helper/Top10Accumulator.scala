package com.atguigu.bigdata.spark.exer.core.req.helper
...
class Top10Accumulator extends AccumulatorV2[(String,String),mutable.Map[String,HotCategory]]{
    ...
    override def add(v: (String, String)): Unit = {
        ...
        val categoryValue: HotCategory = categoryMap.getOrElse(category,HotCategory(category,0,0,0))

        action match {
            case "click" => categoryValue.clickCount += 1
            case "order" => categoryValue.orderCount +=1
            case "pay" => categoryValue.payCount +=1
            case _ =>
        }
